...
while cap.isOpened():
    ...
    if ret: 
        ...
        #repeat()可能会改变tensor的维度。它对tensor中对应repeat参数对应的维度上进行重复给定的次数，如果tensor的维度小于repeat()参数给定的维度，tensor的维度将变成和repeat()一致。这里repeat(1,2)，表示在第一维度上重复一次，第二维上重复两次，repeat(1,2)有2个元素，表示它给定的维度有2个,所以将长度为2的一维行tensor变成了维度为1x4的二维tensor   
                     
        if CUDA:
            im_dim = im_dim.cuda()# 只进行前向计算，不计算梯度
            img = img.cuda()
        ...
        if type(output) == int:
            #将方框属性转换成(ind,x1,y1,x2,y2,s,s_cls,index_cls)，去掉低分，NMS等操作，得到在输入网络坐标系中的最终预测结果
            frames += 1#每次迭代，我们都会跟踪名为frames的变量中帧的数量。然后我们用这个数字除以自第一帧以来过去的时间，得到视频的帧率
        ...
        im_dim = im_dim.repeat(output.size(0), 1)# 得到每个方框所在图片缩放系数
        scaling_factor = torch.min(416/im_dim,1)[0].view(-1,1)# 将方框的坐标(x1,y1,x2,y2)转换为相对于填充后的图片中包含原始图片区域
        ...
        classes = load_classes('data/coco.names')#coco.names文件中保存的是所有类别的名字，load_classes()返回一个列表classes，每个元素是一个类别的名字
        ...
        cv2.imshow("frame", frame)#每次循环，写入该帧
        key = cv2.waitKey(1)
